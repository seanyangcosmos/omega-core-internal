...
for item in evolution_data.get('evolution_events', []):
    if item.get('event_type') == "structure_mutation":
        generated_rules.append({
            ...
            "description": f"偵測到結構變異：{item['description']}，已聯動生成依賴調整規則"
        })
        critical_mutations += 1
    elif item.get('event_type') == "version_extension":
        generated_rules.append({
            "rule_type": "version_consistency_check",
            "description": f"版本擴展：{item['description']}，已聯動生成版本一致性檢核規則"
        })
...
with open('dynamic-rule-linkage-results.json', 'w', encoding='utf-8') as f:
    json.dump(final_output, f, ensure_ascii=False, indent=2)

print("\n📦 動態規則聯動補強已完成，結果輸出至 dynamic-rule-linkage-results.json")
